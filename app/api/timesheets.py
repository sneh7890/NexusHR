...
# from app.utils import ok, err, require_auth, secs_to_hms, secs_to_decimal, today_str
#
# ts_bp = Blueprint("timesheets", __name__)
#
# # Timezone offset map — matches frontend TIMEZONES
# TZ_MAP = {
#     "IST": "Asia/Kolkata",
...
#
#
# def now_in_tz(tz_label):
#     """Return current datetime string in the given timezone."""
#     tz_name = TZ_MAP.get(tz_label, "Asia/Kolkata")
#     try:
...
#
#
# def get_active_ts(user_id, tz_label="IST"):
#     today = today_in_tz(tz_label)
#     return query(
...
#         co_s  = ts.get("clock_out")
#         co    = datetime.datetime.fromisoformat(co_s) if co_s else datetime.datetime.now()
#         total = (co - ci).total_seconds()
#         brks  = break_seconds(ts["id"])
#         if include_active_break:
#             brks += active_break_seconds(ts["id"])
#         return max(0.0, total - brks)
#     except Exception:
#         return 0.0
#
#
...
#     tz   = user.get("timezone", "IST")
#     ts   = get_active_ts(user["id"], tz)
#     if not ts:
...
#     tz_label = data.get("timezone", user.get("timezone", "IST"))
#     today    = today_in_tz(tz_label)
#     now      = now_in_tz(tz_label)
#
#     # Check already clocked in today in this timezone
#     existing = query(
#         "SELECT * FROM timesheets WHERE user_id=? AND date=? AND status='active'",
#         (user["id"], today), fetchone=True
#     )
#     if existing:
...
#         (user["id"], today, now, tz_label)
#     )
#     # Save timezone preference
#     execute("UPDATE users SET timezone=? WHERE id=?", (tz_label, user["id"]))
...
#     execute("UPDATE breaks SET break_out=? WHERE id=?", (now_in_tz(tz), brk["id"]))
...
#     total_secs = sum(r["worked_seconds"] for r in records)
#     return ok({
#         "timesheets":    records,
...
def clock_in():
    ...
    if existing:
        return err("You are already clocked in for today", 409)
...
def clock_out():
    ...
    ts   = get_active_ts(user["id"], tz)
    if not ts:
        return err("You have not clocked in yet. Please clock in first", 400)
    brk = get_active_break(ts["id"])
    now = now_in_tz(tz)
    if brk:
        execute("UPDATE breaks SET break_out=? WHERE id=?", (now, brk["id"]))
...
def break_in():
    ...
    if not ts:
        return err("You have not clocked in yet. Please clock in first", 400)
    if get_active_break(ts["id"]):
        return err("You are already on a break", 409)
    execute_lastid(
        "INSERT INTO breaks (timesheet_id, break_in) VALUES (?,?)",
        (ts["id"], now_in_tz(tz))
    )
...
def break_out():
    ...
    brk = get_active_break(ts["id"])
    if not brk:
        return err("You are not currently on a break", 400)
    execute("UPDATE breaks SET break_out=? WHERE id=?", (now_in_tz(tz), brk["id"]))
    return ok(message="Break ended. Back to work!")
